...
//	}
	...
	public String createMessage(CreateMessageRestModel message) throws Exception {
		...
		ProducerRecord<String, ChatAiCreatedEvent> recordRes = new ProducerRecord<>(env.getProperty("consumer.create.topic"),
				chatId, chEvent);
...
//		// future.join();
	...
	public List<PreparedMessage> listOfMessage(CreateMessageRestModel readMesage) throws Exception {
		...
		List<ProcessedEventEntity> listOfRecord = processEventRepository.findByRecieverIdAndUserIdOrUserIdAndRecieverId(readMesage.getRecieverId(), readMesage.getUserId(), readMesage.getRecieverId(), readMesage.getUserId());
...
//				// future.join();
				...
				LOGGER.info("msq Id ==> {}", msgId);
				LOGGER.info("msq da ==> {}", readMsg.getMessage());
				byte[] messageId = msgId.getBytes();
				//recordRes.headers().add("messageId", messageId);
				
				//SendResult<String, PreparedMessage> result = kafkaReadTemplate.send(recordRes).get();
				
				
				//CreateMessageRestModel upd = new CreateMessageRestModel(result.getProducerRecord().value().getUserId(), result.getProducerRecord().value().getRecieverId(), result.getProducerRecord().value().getMessage(), result.getProducerRecord().value().getChatId());
				updateMessage(evnt);
//				LOGGER.info("Partition ==> " + result.getRecordMetadata().partition());
//				LOGGER.info("Topic ==> " + result.getRecordMetadata().topic());
